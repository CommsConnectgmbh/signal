...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full border-b border-border bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
            ...
            link.dropdown ? (
              <li key={link.label} className="relative"
                ...
                <div className={`absolute left-1/2 top-full pt-2 -translate-x-1/2 transition-all ${
                  ...
                  <ul className="min-w-[220px] rounded-lg border border-border bg-white p-2 shadow-lg">
                    {link.dropdown.map((item) => (
                      <li key={item.href}>
                        {item.extern ? (
                          <a href={item.href} target="_blank" rel="noopener"
                            className="flex items-center justify-between gap-2 rounded-md px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-text-primary">
                            {item.label}
                          ...
                          <Link href={item.href}
                            className="block rounded-md px-4 py-2.5 text-sm text-text-secondary hover:bg-surface hover:text-text-primary">
                            {item.label}
                          </Link>
                        )}
                      </li>
              ...
              <li key={link.label}>
                <Link href={link.href} className="text-sm font-medium text-text-secondary hover:text-text-primary">
                  {link.label}
                </Link>
              </li>
            )
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="relative h-6 w-6 md:hidden" onClick={() => setMobileOpen((p) => !p)}
            ...
            <span className={`absolute left-0 block h-0.5 w-6 bg-text-primary transition-all ${mobileOpen ? "top-3 rotate-45" : "top-1"}`} />
            <span className={`absolute left-0 top-3 block h-0.5 w-6 bg-text-primary transition-all ${mobileOpen ? "opacity-0" : "opacity-100"}`} />
            <span className={`absolute left-0 block h-0.5 w-6 bg-text-primary transition-all ${mobileOpen ? "top-3 -rotate-45" : "top-5"}`} />
          </button>
        </div>
      </div>

      {/* Mobile */}
      <div className={`overflow-hidden border-t border-border bg-white transition-all md:hidden ${mobileOpen ? "max-h-[80vh] overflow-y-auto" : "max-h-0"}`}>
            ...
            link.dropdown ? (
              <li key={link.label}>
                <button
                  ...
                  className="flex w-full items-center justify-between py-2 text-sm font-medium text-text-secondary">
                  {link.label}
                ...
                <ul className={`overflow-hidden pl-4 transition-all ${mobileDropdownOpen ? "max-h-[600px]" : "max-h-0"}`}>
                  <li>
                    <Link href="/produkte" onClick={() => setMobileOpen(false)}
                      className="block py-2 text-sm text-text-secondary">Übersicht</Link>
                  </li>
                  {link.dropdown.map((item) => (
                    <li key={item.href}>
                      {item.extern ? (
                        <a href={item.href} target="_blank" rel="noopener" className="block py-2 text-sm text-text-secondary">
                          {item.label}
                        </a>
                      ) : (
                        <Link href={item.href} onClick={() => setMobileOpen(false)} className="block py-2 text-sm text-text-secondary">
                          {item.label}
                        </Link>
                      )}
                    </li>
